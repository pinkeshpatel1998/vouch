...
import { Badge, Avatar, Skeleton, EmptyState } from "@/components/ui/primitives";
...
export default function Dashboard() {
  ...
  const subtitle =
    count && count.spaces > 0
      ...
      : "One space per product or client.";

  return (
    <main className="mx-auto max-w-4xl px-6 py-8 sm:py-10">
      <header className="mb-7 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-[32px] text-ink">Spaces</h1>
      ...
      {loading ? (
            ...
            {spaces.map((s) => (
              <li key={s.id} style={{ ["--v-accent" as string]: s.accent_color }}>
                <Link
                  href={`/app/${s.id}/inbox`}
                  className={cn(
                    "block h-full rounded-md bg-surface p-4 shadow-low",
                    "transition-[box-shadow,background-color] duration-[220ms] ease-[var(--v-ease-out)]",
                    "hover:bg-hover hover:shadow-mid",
                  )}
                >
                  <div className="flex items-center gap-2.5">
                    <Avatar name={s.name} src={s.logo_url} size={28} className="rounded-md" />
                    <span className="font-display text-[17px] text-ink">{s.name}</span>
                  </div>
                  <div className="mt-3.5 flex flex-wrap gap-1.5">
                    {s.pending > 0 && <Badge tone="pending">{s.pending} pending</Badge>}
                    <Badge tone={s.approved > 0 ? "accent" : "neutral"}>{s.approved} live</Badge>
                  </div>
        ...
        <EmptyState
          ...
          title="No spaces yet"
          body="A space holds one product's testimonials. Create an empty one, or load a demo with thirteen samples to click around in."
          action={
